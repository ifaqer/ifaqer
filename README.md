# 👋 Console.log(`'Matvienko Aleksandr'`)
<p align="center">
...
<div >
	...
	<img width="38.7" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/javascript.png" alt="JavaScript" title="JavaScript"/>
	...
	<img width="45" src="https://raw.githubusercontent.com/marwin1991/profile-technology-icons/refs/heads/main/icons/react_query.png" alt="React Query" title="React Query"/>
...
- Управление контекстом `this` через `call`, `apply`, `bind`, как происходит потеря контекста и как этого не допустить!
...
## 🌩️ Типизация на TypeScript
> Имею опыт работы с TypeScript в проектах различной сложности. Умею работать со следующими концепциями:
- Понимаю базовые типы: `string`, `number`, `boolean`, `null`, `undefined`, `void`, `any`, `unknown`, `never`
- Умею пользоваться: `type`, `interface`, `enum`, `Generics`, `typeof`, `keyof`, 
- Могу использовать утилитарные типы: `Pick`, `Omit`, `Partial`, `Required`, `Readonly`, `Record`
- Работа с массивами `Array<T>` и кортежами `[T1, T2]`, объединение типов `extends`, операторы `&` и `|`
- Понимаю следующие свойства: `?`, `public`, `private`, `protected`
- Использование `declare` для объявления деклараций `.d.ts`
- Владею базовой типизацией React и настройкой tsconfig.json

## 🚀 Библиотека React (v.19):
> Имею практический опыт разработки одностраничных приложений (SPA) на React. Уверенно владею ключевыми концепциями и современными возможностями библиотеки, включая нововведения React 19. Основные компетенции:
...
- Имеется опыт в стилизации проекта: `CSS/SCSS`, `CSS Modules`, `styled-component`, `Tailwind CSS`
...
- Давно использовал `Redux`, сейчас есть практика с `Zustand` и `MobX` менеджерами состояний!
...
## 🐟 Библиотека MobX (State manager)
> Имею опыт использования MobX для управления состоянием в React приложениях. Освоил следующие концепции:
- Основы работы с MobX: создание `store`, `action` и `computed значения`. Создание `makeObservable`, `makeAutoObservable`
- Отслеживание изменений данных через `observer`, композиция хранилищ, и др.
- Что такое `observable`, `action`, `computed`
- Применение `autorun`, `reaction`, `when` для отслеживания реакций на изменения состояния
- Работа с асинхронными действиями (async/await) внутри actions, использование `runInAction`
- Использование утилиты `spy` для отслеживания изменений состояния и действий в MobX
- Использование утилиты `fromPromise` для управления промисами и асинхронными операциями
- Понимаю, как работает механизм реактивности MobX под капотом

## 🧨 Библиотека React Router (v.7)
> Есть опыт работы в __декларативном__ режиме (v.5) для создания и управления роутингом. Знаю следующие концепции:
- Определение маршрутов и их компонентов: `BrowserRouter`, `Routes`, `Route`, `Outlet`, `Link`, `NavLink` `Navigate`
- Использование хуков: `useParams`, `useSearchParams`, `useNavigate`, `useLocation`
- Создание вложенных маршрутов для организации структуры
- Работа с динамическими параметрами в URL
- Реализация защищенных маршрутов, lazy loading, Scroll Restoration
...
## 🔥 Next.JS фреймворк
> На данный момент я изучаю базовые концепции фреймворка и применяю знания для написания дипломной работы на Next.JS. После получение уверенных знаний я заполню этот блок!

## 🤖 Tailwind CSS
>  На данный момент я изучаю базовые концепции Tailwind CSS, начал использовать вместе с Next.JS и shadcn библиотекой. После получение уверенных знаний я заполню этот блок!
